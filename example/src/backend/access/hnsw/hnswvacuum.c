...
NeedsUpdated(HnswVacuumState *vacuumstate, HnswElement element)
{
	...
	LockBuffer(buf, BUFFER_LOCK_SHARE); // 加共享锁
	...
	for (int i = 0; i < ntup->count; i++)
	{
		...
		if (DeletedContains(vacuumstate->deleted, indextid)) // 检查是否在删除列表中
		{
			needsUpdated = true;
			break;
		}
...
RepairGraphElement(HnswVacuumState *vacuumstate, HnswElement element, HnswElement entryPoint)
{
	...
	// 提示2：初始化邻居结构
	HnswInitNeighbors(base, element, m, NULL); // 初始化邻居
	element->heaptidsLength = 0;

	// 提示3：查找新邻居，注意最后一个参数表示是否跳过自身
	HnswFindElementNeighbors(base, element, entryPoint, index, support, m, efConstruction, true); // 查找邻居

	// 提示4：更新邻居元组
	MemSet(ntup, 0, HNSW_TUPLE_ALLOC_SIZE);
	HnswSetNeighborTuple(base, ntup, element, m); // 设置邻居元组

// 该文件中的这些题目覆盖了HNSW索引构建过程中的关键概念和实现细节，包括：
// 索引文件结构（魔数、版本号、元组格式）
// 内存管理（内存分配、内存使用跟踪、内存刷新）
// 并发控制（锁机制、并行构建协调）
// HNSW算法核心参数（M、efConstruction、层管理）
// 数据持久化（内存数据到磁盘的转换）
// 索引构建流程（从扫描表到创建索引结构）
...
CreateMetaPage(HnswBuildState *buildstate) {
    ...
    /* 第1题：设置HNSW索引的魔数，用于验证索引文件的有效性 */
    /* 提示：魔数是一个预定义的常量，用于标识HNSW索引文件格式，确保索引的正确性和兼容性 */
    metap->magicNumber = HNSW_MAGIC_NUMBER;  // 第1题

    /* 第2题：设置元数据页的版本号 */
    /* 提示：版本号用于处理索引格式的演进，确保不同版本的PostgreSQL能够正确识别和处理索引文件 */
    metap->version = HNSW_VERSION;  // 第2题
...
CreateGraphPages(HnswBuildState *buildstate) {
    ...
    while (!HnswPtrIsNull(base, iter)) {
        ...
        /* 第3题：计算HNSW元素元组的大小 */
        /* 提示：此宏根据向量值的实际大小计算存储一个HNSW元素元组所需的总内存空间，包括向量数据和元数据 */
        etupSize = HNSW_ELEMENT_TUPLE_SIZE(VARSIZE_ANY(valuePtr));  // 第3题
        ...
        /* 第4题：将HnswElement结构体的内容复制到HnswElementTuple中 */
        /* 提示：此函数用于将内存中的HNSW元素转换为适合存储到磁盘的元组格式，是内存数据持久化的关键步骤 */
        HnswSetElementTuple(base, etup, element);  // 第4题
...
WriteNeighborTuples(HnswBuildState *buildstate) {
    ...
    while (!HnswPtrIsNull(base, iter)) {
        ...
        /* 第5题：将元素的邻居信息复制到HnswNeighborTuple中 */
        /* 提示：此函数用于将元素的邻居列表转换为适合存储到磁盘的格式，包含了该元素在不同层的所有邻居信息 */
        HnswSetNeighborTuple(base, ntup, element, m);  // 第5题
...
FindDuplicateInMemory(char *base, HnswElement element) {
    ...
    for (int i = 0; i < neighbors->length; i++) {
        ...
        /* 第6题：比较两个Datum值是否相等 */
        /* 提示：此函数用于精确比较两个向量值是否完全相同，是检测重复元素的核心逻辑 */
        if (!datumIsEqual(value, neighborValue, false, -1))  // 第6题
            return false;
...
UpdateNeighborsInMemory(char *base, HnswSupport *support, HnswElement e, int m) {
    for (int lc = e->level; lc >= 0; lc--) {
        /* 第7题：获取指定层的M值（邻居数量） */
        /* 提示：HNSW索引中不同层可以有不同的M值，此函数根据当前层和基础M值计算实际使用的邻居数量 */
        int lm = HnswGetLayerM(m, lc);  // 第7题
        ...
        for (int i = 0; i < neighbors->length; i++) {
            ...
            HnswUpdateConnection(base, HnswGetNeighbors(base, neighborElement, lc), e, hc->distance, lm, NULL, NULL,
                                 support);
 ...
 */
static void
UpdateGraphInMemory(HnswSupport *support, HnswElement element, int m, int efConstruction, HnswElement entryPoint,
...
InsertTuple(Relation index, Datum *values, bool *isnull, ItemPointer heaptid, HnswBuildState *buildstate) {
    ...
    /* Form index value */
    /* 第8题：将输入的Datum值转换为适合HNSW索引存储和比较的格式 */
    /* 提示：此函数处理不同类型的输入数据（如不同维度的向量），将其统一转换为HNSW索引内部使用的格式 */
    if (!HnswFormIndexValue(&value, values, isnull, buildstate->typeInfo, support))  // 第8题
        return false;
    ...
    /* 第9题：检查当前已使用内存是否超过总内存限制 */
    /* 提示：此变量跟踪构建过程中已经使用的内存量，用于决定是否需要将部分图结构刷新到磁盘以释放内存 */
    if (graph->memoryUsed >= graph->memoryTotal)  // 第9题
    {
    ...
    /* 第10题：初始化一个新的HnswElement结构体 */
    /* 提示：此函数创建并初始化一个新的HNSW元素，分配必要的内存空间，并设置初始状态 */
    element = HnswInitElement(base, heaptid, buildstate->m, buildstate->ml, buildstate->maxLevel, allocator);  // 第10题
    ...
    /* 第11题：初始化元素的锁 */
    /* 提示：此函数为新创建的HNSW元素初始化一个锁，用于多进程并发访问时的同步控制 */
    LWLockInitialize(&element->lock, hnsw_lock_tranche_id);  // 第11题
...
InitGraph(HnswGraph *graph, char *base, Size memoryTotal) {
    ...
    /* 第12题：存储图的头指针 */
    /* 提示：此宏用于在HNSW的相对指针系统中存储指针值，是实现高效内存管理和并行访问的关键 */
    HnswPtrStore(base, graph->head, (HnswElement) NULL);  // 第12题

    /* 第13题：存储图的入口点指针 */
    /* 提示：入口点是HNSW索引的起始搜索位置，通常是最高层的第一个元素，此宏用于正确存储这个指针 */
    HnswPtrStore(base, graph->entryPoint, (HnswElement) NULL);  // 第13题
    graph->memoryUsed = 0;
    ...
    /* 第14题：初始化图的刷新锁 */
    /* 提示：刷新锁用于控制何时将内存中的图结构刷新到磁盘，确保并发访问时的数据一致性 */
    LWLockInitialize(&graph->flushLock, hnsw_lock_tranche_id);  // 第14题
}
...
InitBuildState(HnswBuildState *buildstate, Relation heap, Relation index, IndexInfo *indexInfo, ForkNumber forkNum) {
    ...
    /* 第15题：计算ml参数（层分布参数） */
    /* 提示：ml参数用于确定新元素应该插入到HNSW索引的哪一层，影响索引的搜索性能和构建效率 */
    buildstate->ml = HnswGetMl(buildstate->m);  // 第15题
...
HnswParallelScanAndInsert(Relation heapRel, Relation indexRel, HnswShared *hnswshared, char *hnswarea, bool progress) {
    ...
    /* 第16题：执行表索引构建扫描 */
    /* 提示：此函数是PostgreSQL表访问方法的一部分，用于扫描表并为每个元组调用指定的回调函数进行索引构建 */
    reltuples = table_index_build_scan(heapRel, indexRel, indexInfo, true, progress, BuildCallback, (void *) &buildstate, scan);  // 第16题
...
HnswParallelBuildMain(dsm_segment *seg, shm_toc *toc) {
    ...
    /* 第17题：设置调试查询字符串 */
    /* 提示：此全局变量存储当前正在执行的查询字符串，用于调试和性能分析目的 */
    debug_query_string = sharedquery;  // 第17题
...
HnswBeginParallel(HnswBuildState *buildstate, bool isconcurrent, int request) {
    ...
    /* 第18题：估计并行构建HNSW索引所需的共享内存大小 */
    /* 提示：此函数计算并行构建过程中需要的共享内存量，确保有足够的内存空间用于协调多个工作进程 */
    esthnswshared = ParallelEstimateShared(buildstate->heap, snapshot);  // 第18题
    ...
    /* 第19题：初始化共享内存中的图数据结构 */
    /* 提示：此函数在并行构建场景下初始化共享内存中的HNSW图，设置初始状态和参数 */
    InitGraph(&hnswshared->graphData, hnswarea, esthnswarea - 1024 * 1024);  // 第19题
...
BuildGraph(HnswBuildState *buildstate, ForkNumber forkNum) {
    ...
    if (buildstate->heap != NULL)
        /* 第20题：计算构建索引时可以使用的并行工作进程数 */
        /* 提示：此函数综合考虑系统配置、表属性和索引类型，确定最佳的并行工作进程数量 */
        parallel_workers = ComputeParallelWorkers(buildstate->heap, buildstate->index);  // 第20题
